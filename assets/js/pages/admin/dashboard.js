...
// dashboard.html의 initializeDashboardPage()가 window.initDashboard()를 호출하므로
// 여기서 직접 호출하면 이중 초기화가 발생한다.
...
async function checkAdminAccess(user = null) {
    ...
    try {
        ...
        if (window.adminAuth && typeof window.adminAuth.isAdmin === 'function') {
            // Firestore에서 관리자 상태를 먼저 로드 (캐시가 없으면 false 반환하는 문제 방지)
            if (typeof window.adminAuth.initAdminStatus === 'function') {
                await window.adminAuth.initAdminStatus();
            }
