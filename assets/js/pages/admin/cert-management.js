...
Object.assign(window.certManager, {
    ...
    async generateCertificateNumber() {
        ...
        await window.dhcFirebase.db.runTransaction(async (tx) => {
            ...
            // .count 우선, 기존 .value 필드 fallback으로 마이그레이션 처리
            const current = snap.exists ? (snap.data().count ?? snap.data().value ?? 0) : 0;
    ...
    async previewNextCertNumber() {
        const year = new Date().getFullYear();
        const certTypeCode = this.getCertTypeCode(this.currentCertType);
        const counterId = `cert_${this.currentCertType}_${year}`;

        if (!checkFirebaseConnection().connected || !window.dhcFirebase) {
            return `${certTypeCode}-${year}-XXXX`;
        }

        const snap = await window.dhcFirebase.db.collection('_counters').doc(counterId).get();
        const current = snap.exists ? (snap.data().count ?? snap.data().value ?? 0) : 0;
        return `${certTypeCode}-${year}-${(current + 1).toString().padStart(4, '0')}`;
    },

    async checkCertNumberDuplicate(certNumber) {
        if (!checkFirebaseConnection().connected || !window.dhcFirebase) return false;
        const snap = await window.dhcFirebase.db.collection('certificates')
    ...
    async regenerateCertNumber() {
        const field = document.getElementById('issue-cert-number');
        if (!field) return;
        field.value = '생성 중...';
        const preview = await this.previewNextCertNumber();
        field.value = preview;
...
Object.assign(window.certManager, {
    ...
    showIssueCertModal() {
        ...
        if (modal) {
            ...
            // 자격증 번호 pre-fill (카운터 증가 없는 peek)
            const certNumberField = document.getElementById('issue-cert-number');
            if (certNumberField) {
                certNumberField.value = '생성 중...';
                certNumberField.dataset.autoGenerated = 'true';
                this.previewNextCertNumber().then(preview => {
                    certNumberField.value = preview;
                });
                certNumberField.addEventListener('input', function () {
                    certNumberField.dataset.autoGenerated = 'false';
                }, { once: true });
            }
        }
    },
    ...
    showPaidApplicantsModal() {
        ...
        if (modal) {
            ...
            if (certTypeName) certTypeName.textContent = this.getCertTypeName(this.currentCertType);

            this.loadPaidApplicants();
            this.setDefaultBulkDates();

            // 시작 번호 pre-fill
            const startField = document.getElementById('bulk-start-cert-number');
            if (startField) {
                startField.value = '생성 중...';
                startField.dataset.autoGenerated = 'true';
                this.previewNextCertNumber().then(preview => { startField.value = preview; });
                startField.addEventListener('input', function () {
                    startField.dataset.autoGenerated = 'false';
                }, { once: true });
            }
        }
    },

    async regenerateBulkStartNumber() {
        const field = document.getElementById('bulk-start-cert-number');
        if (!field) return;
        field.value = '생성 중...';
        const preview = await this.previewNextCertNumber();
        field.value = preview;
    ...
    incrementCertNumber(certNumber, delta) {
        const match = certNumber.match(/^(.+-)(\d+)$/);
        if (!match) return certNumber;
        const num = parseInt(match[2], 10) + delta;
        return `${match[1]}${num.toString().padStart(match[2].length, '0')}`;
    },
    ...
    async issueSelectedCertificates() {
        ...
        const issueDateVal  = document.getElementById('bulk-issue-date')?.value;
        const expiryDateVal = document.getElementById('bulk-expiry-date')?.value;
        const startField    = document.getElementById('bulk-start-cert-number');
        ...
        if (new Date(issueDateVal) >= new Date(expiryDateVal)) {
            window.adminAuth?.showNotification('만료일은 발급일보다 이후여야 합니다.', 'warning');
            return;
        }

        const isAutoGenerated = !startField || startField.dataset.autoGenerated !== 'false';
        const manualStartNumber = startField?.value?.trim() || '';

        if (!isAutoGenerated && !manualStartNumber) {
            window.adminAuth?.showNotification('시작 자격증 번호를 입력해주세요.', 'warning');
            return;
        }

        const count = selectedCheckboxes.length;
        if (!confirm(`선택된 ${count}명의 신청자에게 자격증을 발급하시겠습니까?`)) return;
        ...
        try {
            ...
            if (isAutoGenerated) {
                // 자동생성: 각 신청자마다 순차 발급 (원자적 카운터)
                for (let i = 0; i < selectedIds.length; i++) {
                    certNumbers.push(await this.generateCertificateNumber());
                }
            } else {
                // 수동 시작 번호: 중복 체크 후 순번 증가
                for (let i = 0; i < selectedIds.length; i++) {
                    const num = this.incrementCertNumber(manualStartNumber, i);
                    const isDuplicate = await this.checkCertNumberDuplicate(num);
                    if (isDuplicate) {
                        window.adminAuth?.showNotification(
                            `자격증 번호 "${num}"는 이미 사용 중입니다. 시작 번호를 확인하세요.`, 'warning'
                        );
                        return;
                    }
            ...
            // Firestore batch로 일괄 저장
            const batch = window.dhcFirebase.db.batch();
            selectedIds.forEach((enrollmentId, idx) => {
                const applicant = this.allPaidApplicants.find(a => a.id === enrollmentId) || {};
                const certRef = window.dhcFirebase.db.collection('certificates').doc();
                batch.set(certRef, {
                    certificateNumber: certNumbers[idx],
                    certNumber:        certNumbers[idx],
                    holderName:        applicant.name || '',
                    holderNameKorean:  applicant.name || '',
                    holderNameEnglish: this.generateEnglishName(applicant.name || ''),
                    holderEmail:       applicant.email || '',
                    certificateType:   this.currentCertType,
                    courseName:        applicant.courseName || '',
                    enrollmentId,
                    issueDate:         FS.Timestamp.fromDate(issueDate),
                    expiryDate:        FS.Timestamp.fromDate(expiryDate),
                    status:            'active',
                    isIssued:          true,
                    issueMethod:       'paid-applicants',
            ...
            window.adminAuth?.showNotification(
                `${count}명의 자격증이 성공적으로 발급되었습니다. (${certNumbers[0]} ~ ${certNumbers[count - 1]})`,
                'success'
            );
            this.closeModalById('paid-applicants-modal');
            this.loadCertificatesData();
            ...
            window.adminAuth?.showNotification('자격증 발급 중 오류가 발생했습니다: ' + error.message, 'error');
        }
    },
...
Object.assign(window.certManager, {
    ...
    async issueCertificate(formElement) {
        try {
            ...
            const certNumberField = document.getElementById('issue-cert-number');
            const issueData = {
                certNumber: formData.get('certNumber')?.trim() || '',
                certNumberAutoGenerated: certNumberField?.dataset.autoGenerated !== 'false',
            ...
            let certNumber;
            if (!issueData.certNumber || issueData.certNumberAutoGenerated) {
                // 자동 생성 (카운터 원자적 증가)
                certNumber = await this.generateCertificateNumber();
            } else {
                // 관리자 직접 입력 — 중복 확인
                certNumber = issueData.certNumber;
                const isDuplicate = await this.checkCertNumberDuplicate(certNumber);
                if (isDuplicate) {
                    window.adminAuth?.showNotification(`자격증 번호 "${certNumber}"는 이미 사용 중입니다.`, 'warning');
                    return;
                }
            }
    ...
    validateIssueData(data) {
        ...
        if (!data.certNumberAutoGenerated && data.certNumber) {
            // 수동 입력 시 형식 기본 검사 (빈 문자열이 아닌 경우만)
            if (data.certNumber.length < 3) {
                window.adminAuth?.showNotification('자격증 번호를 올바르게 입력해주세요.', 'warning');
                return false;
            }
        }
    ...
    async approveApplication(applicationId) {
        ...
        // 직접 처리 대신 확인 모달 열기
        await this.showApprovalConfirmModal(applicationId);
    },

    async showApprovalConfirmModal(applicationId) {
        const modal = document.getElementById('approval-confirm-modal');
        if (!modal) return;

        // applicationId 저장
        document.getElementById('approval-application-id').value = applicationId;

        // 신청자 이름 조회
        let holderName = '조회 중...';
        try {
            if (checkFirebaseConnection().connected && window.dhcFirebase) {
                const snap = await window.dhcFirebase.db.collection('certificates').doc(applicationId).get();
                if (snap.exists) {
                    const d = snap.data();
                    holderName = d.holderName || d.holderNameKorean || d['name-korean'] || '-';
                }
            }
        } catch (e) { holderName = '-'; }

        document.getElementById('approval-holder-name').textContent = holderName;

        // 자격증 번호 pre-fill
        const certNumberField = document.getElementById('approval-cert-number');
        certNumberField.value = '생성 중...';
        certNumberField.dataset.autoGenerated = 'true';
        this.previewNextCertNumber().then(preview => {
            certNumberField.value = preview;
        ...
        // 날짜 기본값
        const today = new Date();
        const issueDateField = document.getElementById('approval-issue-date');
        const expiryDateField = document.getElementById('approval-expiry-date');
        if (issueDateField) issueDateField.value = window.formatters.formatDate(today, 'YYYY-MM-DD');
        if (expiryDateField) {
            const expiry = window.dateUtils.addYears(today, 3);
            expiryDateField.value = window.formatters.formatDate(expiry, 'YYYY-MM-DD');
        }

        this.closeOtherModals('approval-confirm-modal');
        this.modalStates['approval-confirm-modal'] = true;
        modal.classList.remove('hidden');
        document.body.classList.add('modal-open');
    },

    closeApprovalConfirmModal() {
        this.closeModalById('approval-confirm-modal');
    },

    syncApprovalExpiryDate() {
        const issueDateVal = document.getElementById('approval-issue-date')?.value;
        if (!issueDateVal) return;
        const expiry = window.dateUtils.addYears(new Date(issueDateVal), 3);
        const expiryField = document.getElementById('approval-expiry-date');
        if (expiryField) expiryField.value = window.formatters.formatDate(expiry, 'YYYY-MM-DD');
    },

    async regenerateApprovalCertNumber() {
        const field = document.getElementById('approval-cert-number');
        if (!field) return;
        field.value = '생성 중...';
        const preview = await this.previewNextCertNumber();
        field.value = preview;
    ...
    async confirmApproval() {
        ...
        const certNumberField = document.getElementById('approval-cert-number');
        const issueDateVal    = document.getElementById('approval-issue-date')?.value;
        const expiryDateVal   = document.getElementById('approval-expiry-date')?.value;

        if (!certNumberField?.value || !issueDateVal || !expiryDateVal) {
            window.adminAuth?.showNotification('모든 필드를 입력해주세요.', 'warning');
            return;
        }
        ...
        try {
            ...
            const isAutoGenerated = certNumberField.dataset.autoGenerated !== 'false';
            let certNumber;
            ...
            if (isAutoGenerated) {
                // 자동생성: 카운터 증가 + 승인 업데이트를 단일 트랜잭션으로
                const year = new Date().getFullYear();
                const certTypeCode = this.getCertTypeCode(this.currentCertType);
                const counterId = `cert_${this.currentCertType}_${year}`;
                const counterRef = window.dhcFirebase.db.collection('_counters').doc(counterId);
                ...
                await window.dhcFirebase.db.runTransaction(async (tx) => {
                    ...
                    if (appSnap.data().applicationStatus === 'approved') { alreadyApproved = true; return; }

                    const current = counterSnap.exists ? (counterSnap.data().count ?? counterSnap.data().value ?? 0) : 0;
                    const nextNum = current + 1;
                    certNumber = `${certTypeCode}-${year}-${nextNum.toString().padStart(4, '0')}`;

                    tx.set(counterRef, { count: nextNum }, { merge: true });
                    tx.update(certDocRef, {
                        isIssued: true, needsApproval: false, applicationStatus: 'approved', status: 'active',
                        certificateNumber: certNumber,
                        issueDate:  FS.Timestamp.fromDate(issueDate),
                        expiryDate: FS.Timestamp.fromDate(expiryDate),
                        ...
                        approvedBy: 'admin',
                        updatedAt:  FS.FieldValue.serverTimestamp(),
                        remarks: `[${issueDate.toLocaleString('ko-KR')}] 관리자 승인 및 발급 완료`
                    });
                });
            } else {
                // 수동 입력: 중복 확인 후 업데이트
                certNumber = certNumberField.value.trim();
                const isDuplicate = await this.checkCertNumberDuplicate(certNumber);
                if (isDuplicate) {
                    window.adminAuth?.showNotification(`자격증 번호 "${certNumber}"는 이미 사용 중입니다.`, 'warning');
                    return;
                ...
                await window.dhcFirebase.db.runTransaction(async (tx) => {
                    const appSnap = await tx.get(certDocRef);
                    if (!appSnap.exists) throw new Error('신청서를 찾을 수 없습니다.');
                    if (appSnap.data().applicationStatus === 'approved') { alreadyApproved = true; return; }
            ...
            window.adminAuth?.showNotification(
                `자격증이 발급되었습니다. (번호: ${certNumber})`, 'success'
            );
            this.closeApprovalConfirmModal();
            await this.loadCertificatesData();

        } catch (error) {
            console.error('❌ 발급 확정 오류:', error);
            window.adminAuth?.showNotification(`발급 중 오류: ${error.message}`, 'error');
        } finally {
